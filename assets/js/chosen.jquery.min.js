...
(function() {
	...
	n = this, e = jQuery, e.fn.extend({chosen: function(n) {
		...
		return i(r, t), r.prototype.setup = function() {
			...
			var t, n;
			return this.container_id = this.form_field.id.length ? this.form_field.id.replace(/[^\w]/g, "_") : this.generate_field_id(), this.container_id += "_chzn", t = ["chzn-container"], t.push("chzn-container-" + (this.is_multiple ? "multi" : "single")), this.inherit_select_classes && this.form_field.className && t.push(this.form_field.className), this.is_rtl && t.push("chzn-rtl"), n = {id: this.container_id, "class": t.join(" "), style: "width: " + this.container_width() + ";", title: this.form_field.title}, this.container = e("<div />", n), this.is_multiple ? this.container.html('<ul class="chzn-choices"><li class="search-field"><input type="text" value="' + this.default_text + '" class="default" autocomplete="off" style="width:25px;" /></li></ul><div class="chzn-drop"><ul class="chzn-results"></ul></div>') : this.container.html('<a href="javascript:void(0)" class="chzn-single chzn-default" tabindex="-1"><span>' + this.default_text + '</span><div><b></b></div></a><div class="chzn-drop"><div class="chzn-search"><input type="text" autocomplete="off" /></div><ul class="chzn-results"></ul></div>'), this.form_field_jq.hide().after(this.container), this.dropdown = this.container.find("div.chzn-drop").first(), this.search_field = this.container.find("input").first(), this.search_results = this.container.find("ul.chzn-results").first(), this.search_field_scale(), this.search_no_results = this.container.find("li.no-results").first(), this.is_multiple ? (this.search_choices = this.container.find("ul.chzn-choices").first(), this.search_container = this.container.find("li.search-field").first()) : (this.search_container = this.container.find("div.chzn-search").first(), this.selected_item = this.container.find(".chzn-single").first()), this.results_build(), this.set_tab_index(), this.set_label_behavior(), this.form_field_jq.trigger("liszt:ready", {chosen: this})
			...
			if (!this.is_disabled) {
				...
				if (t == null || !e(t.target).hasClass("search-choice-close"))
					return this.active_field ? !this.is_multiple && t && (e(t.target)[0] === this.selected_item[0] || e(t.target).parents("a.chzn-single").length) && (t.preventDefault(), this.results_toggle()) : (this.is_multiple && this.search_field.val(""), e(document).click(this.click_test_action), this.results_show()), this.activate_field()
			}
			...
			if (t != null)
				return e.preventDefault(), e.type === "DOMMouseScroll" && (t *= 40), this.search_results.scrollTop(t + this.search_results.scrollTop())
			...
			var e, t, r, i, s;
			this.parsing = !0, this.results_data = n.SelectParser.select_to_array(this.form_field), this.is_multiple && this.choices > 0 ? (this.search_choices.find("li.search-choice").remove(), this.choices = 0) : this.is_multiple || (this.selected_item.addClass("chzn-default").find("span").text(this.default_text), this.create_option && !this.disable_search ? this.container.removeClass("chzn-container-single-nosearch") : (this.disable_search || this.form_field.options.length <= this.disable_search_threshold) && this.container.addClass("chzn-container-single-nosearch")), e = "", s = this.results_data;
			for (r = 0, i = s.length; r < i; r++)
				t = s[r], t.group ? e += this.result_add_group(t) : t.empty || (e += this.result_add_option(t), t.selected && this.is_multiple ? this.choice_build(t) : t.selected && !this.is_multiple && (this.selected_item.removeClass("chzn-default").find("span").text(t.text), this.allow_single_deselect && this.single_deselect_control_build()));
			...
			if (this.form_field_label.length > 0)
				return this.form_field_label.click(function(e) {
			...
			this.no_results_clear(), this.create_option_clear(), c = 0, n = !1, h = this.search_field.val() === this.default_text ? "" : e("<div/>").text(e.trim(this.search_field.val())).html(), a = this.search_contains ? "" : "^", u = new RegExp(a + h.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&"), "i"), v = new RegExp(h.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&"), "i"), t = new RegExp("^" + h.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&") + "$", "i"), w = this.results_data;
			for (m = 0, y = w.length; m < y; m++) {
				i = w[m];
				if (!i.disabled && !i.empty)
					if (i.group)
						e("#" + i.dom_id).css("display", "none");
					else if (!this.is_multiple || !i.selected) {
						r = !1, l = i.dom_id, f = e("#" + l);
						if (u.test(i.html))
							r = !0, c += 1, t.test(i.html) && (n = !0);
						else if (this.enable_split_word_search && (i.html.indexOf(" ") >= 0 || i.html.indexOf("[") === 0)) {
							o = i.html.replace(/\[|\]/g, "").split(" ");
							if (o.length)
								for (g = 0, b = o.length; g < b; g++)
									s = o[g], u.test(s) && (r = !0, c += 1)
						}
						r ? (h.length ? (p = i.html.search(v), d = i.html.substr(0, p + h.length) + "</em>" + i.html.substr(p + h.length), d = d.substr(0, p) + "<em>" + d.substr(p)) : d = i.html, f.html(d), this.result_activate(f), i.group_array_index != null && e("#" + this.results_data[i.group_array_index].dom_id).css("display", "list-item")) : (this.result_highlight && l === this.result_highlight.attr("id") && this.result_clear_highlight(), this.result_deactivate(f))
					}
			}
			c < 1 && h.length ? (!this.create_option || !this.skip_no_results) && this.no_results(h) : this.winnow_results_set_highlight();
			if (this.create_option && (c < 1 || !n && this.persistent_create_option) && h.length)
				return this.show_create_option(h)
		...
		}, r.prototype.show_create_option = function(t) {
			var n;
			return n = e('<li class="create-option active-result"><a href="javascript:void(0);">z' + this.create_option_text + '</a>: "' + t + '"</li>'), this.search_results.append(n)
		}, r.prototype.create_option_clear = function() {
			return this.search_results.find(".create-option").remove()
		}, r.prototype.select_create_option = function(t) {
			return e.isFunction(this.create_option) ? this.create_option.call(this, t) : this.select_append_option({value: t, text: t})
		}, r.prototype.select_create_option_add = function(t) {
			var ret = e.isFunction(this.create_option) ? this.create_option.call(this, t) : this.select_append_option({value: "QRD@" + t, text: t});
			this.form_field_jq.trigger("change");
			return ret ;
		}, r.prototype.select_append_option = function(t) {
			var n, r;
			return n = e("<option />", t).attr("selected", "selected"), this.form_field_jq.append(n), r = this.search_field.val(), this.form_field_jq.trigger("liszt:updated"), this.search_field.trigger("focus")
