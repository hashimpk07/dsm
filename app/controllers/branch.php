<?php
class Branch extends Controller
{
	public function __construct()	{

		$acls = array(
			'allow' => array(
						'*' => '*' ),
			'deny' => array(),
			'order' => 'AD', //Allow, Then Deny (Options are "DA" or "AD")
			'user' => true
		);
		$this->acl($acls) ;

		parent::__construct();

		/* @var $this->brokers_model Brokers_model */
		$this->loadModel('branch_model') ;
	}
	/**
	 * Default function
	 */
	function index()
	{
		//request will be routed to display.
		parent::defIndex() ;
	}
	/**
	 * All search reach here
	 */
	function search()
	{
		$this->listtable() ;
	}
	/*
	 * Ajax page view
	 */
	function page()
	{
		$this->listall();
	}
	/**
	 * Display view without design layout.
	 */
	function listall()
	{
		parent::defListall() ;
	}
	/**
	 * Method to retrieve records list table.
	 * 
	 * @param int $page page no.
	 */
	function listtable($page=1)
	{
		$this->loadLibrary('pagination.php') ;
		$filter = $this->input->request('searchq') ;
		if( ! $filter )
		{
			$filter = $this->input->request('hid-searchq') ;
		}
		$cond = '' ;
		if( $filter )
		{
			$cond = " AND (b.name LIKE '%$filter%' OR b.code LIKE '%$filter%' OR b.name LIKE '%$filter%')" ;
		}

		//{sort
		$sort = '' ;
		if( @$this->input->request['searchq-col'] || @$this->input->request['hid-searchq-col'] )
		{
			$sort = ' ORDER BY ' . ( ($this->input->request['searchq-col']) ? $this->input->request['searchq-col'] : @$this->input->request['hid-searchq-col'] ) ;
			if( (@$this->input->request['searchq-sort'] == 'desc') || (@$this->input->request['hid-searchq-sort'] == 'desc') )
			{
				$sort .= ' DESC ' ;
			}
			else
			{
				$sort .= ' ASC ' ;
			}
		}
		if( ! $sort )
		{
			$sort = ' ORDER BY b.id DESC ' ;
		}

		//Branch only data..
		$branchId = branchEmployee() ;
		if( $branchId )
		{
			$cond .= " AND b.id='$branchId' " ; 
		}
		
		$sql = "SELECT b.id, b.code, b.name,s.name as screen FROM branch b
				LEFT JOIN screen s ON s.id = b.screen_id
				WHERE b.deleted='0' $cond $sort" ;
		
		$sqlcnt ="SELECT COUNT(*) as cnt FROM branch b
				LEFT JOIN screen s ON s.id = b.screen_id
				WHERE b.deleted='0' $cond" ;

		$url = siteUrl('branch/listtable/') ;

        $this->vars['pager_url'] = $url ;
		if( $this->ifCsvExport() )
		{
			$order = array(
				'Branch' => 'branch',
				'Screen'=>'screen',
			) ;

			$this->exportCsv($sql, $order) ;
			return;
		}
		
		$result = $this->pagination->pager($sql, $sqlcnt, $url, 'idListArea' . get_class($this), $page) ;
		//render page with default template file.
		parent::defListtable($result) ;
	}
	/**
	 * Validation function for add and edit.
	 * 
	 * @param string 'e' for edit, 'a' for add
	 * @return array validation results..
	 */
	function validate($mode)
	{
		$errors = array() ;
		$errors['eName'] = '' ;
		$errors['eCode'] = '' ;
		
		//{ db check conditions
		if( $mode == 'edit' )
		{
			$editId = $this->getArg('editId') ;
			$eCond = " ( name = '" . $this->fields['txtName'] . "' AND id != '$editId' AND deleted = 0 ) " ;
			$eCodeCond = " ( code = '" . $this->fields['txtCode'] . "' AND id != '$editId' AND deleted = 0 ) " ;
			
		}
		else if( $mode == 'add' )
		{
			$eCond = " ( name = '" . $this->fields['txtName'] . "' AND deleted = 0 ) " ;
			$eCodeCond = " ( code = '" . $this->fields['txtCode'] . "' AND deleted = 0 ) " ;
		}
		//}
		
		//QC_LIMIT: check branch limit..
		$sql = "SELECT COUNT(*) as cnt FROM branch WHERE deleted=0" ;
		$max = $this->db->scalarField($sql) ;

		if( $max >= QC_LIMIT || ($max*2) >= QC_LIMIT_2X )
		{
			$errors['eName'] = 'Unable to create branch' ;
		}

		//basic test cases		
		if( ! @$this->fields['txtName'] )
		{
			$errors['eName'] = 'Name not specified' ;
		}
		//basic test cases		
		if( ! @$this->fields['txtCode'] )
		{
			$errors['eCode'] = 'Code not specified' ;
		}
		//db test cases
		if( @$this->fields['txtName'] )
		{
			if( $this->branch_model->isExists($eCond) )
			{
				$errors['eName'] = 'Name already exists' ;
			}
		}
		if( @$this->fields['txtCode'] )
		{
			if( $this->branch_model->isExists($eCodeCond) )
			{
				$errors['eCode'] = 'Code already exists' ;
			}
		}
		return $errors ;
	}
	/**
	 * Function to handle add form submition.
	 * 
	 * @return boolean true on success
	 */
	private function onAdd()
	{ 
		$errors = $this->validate('add') ;
		 
		//has any error ?
		if( countReal($errors) > 0 )
		{ 
			//then stop here..
			return new JStatus(false, 'Please fix validation errors', $errors) ;
			//--- END: ----
		}
	
		$this->db->beginTrans() ;
		
		$branch = array(
			    'name' => $this->fields['txtName'],
			    'code' => $this->fields['txtCode'],
				'screen_id' => sqlNullableKey($this->fields['cbScreen']),
				'city_id' => sqlNullableKey($this->fields['selCity']),
		) ;

		if( $this->branch_model->insert($branch) )
		{	
			$insId = $this->db->getLastInsertId() ;
			$this->db->commitTrans() ;

			return new JStatus(true, 'Successfully saved', array('__id' => $insId)) ;
			 
		}
		 
		$this->db->rollbackTrans() ;
		return new JStatus(false, 'Unable to save') ;
	}
	/**
	 * Shows add form
	 */
	function add()
	{
		if( $this->input->post('btnSubmit') )
		{
			$jstat = $this->onAdd() ;
			
			if( $jstat->status )
			{
				if( $this->getArg('contentAreaClicked') != 'idPopupSubmit' )
				{
					ob_start() ;
					if( $this->getArg('contentAreaClicked') == 'idContentAreaSmall' )
					{
						$this->view($jstat->data['__id']) ;					
					}
					else
					{
						$this->page() ;
					}
					$jstat->data['idContentAreaBig'] = ob_get_clean() ;
				}
			}
			$this->statusResponse($jstat) ;
			return ;
		}
		$this->vars['screens']=$this->getModel('screen_model')->get();
		$this->vars['countries']=$this->getModel('country_model')->get();

		$this->vars['mode'] = 'add' ;
		$this->vars['url'] = siteUrl('branch/add') ;
		parent::defAdd() ;
	}
	/**
	 * On edit submit
	 */
	private function onEdit($id)
	{
		$errors = $this->validate('edit') ;
		//has any error ?
		if( countReal($errors) > 0 )
		{
			//then stop here..
			return new JStatus(false, 'Please fix validation errors', $errors) ;
			//--- END ---
		}

		//get id
        $this->db->beginTrans() ;
		//get current values 
		$data = $this->branch_model->getDetails($id) ;

		$branch = array(
			    'name' => $this->fields['txtName'],
				'code' => $this->fields['txtCode'],
				'screen_id' => sqlNullableKey($this->fields['cbScreen']),
				'city_id' => sqlNullableKey($this->fields['selCity']),
		) ;
		
		if( $this->branch_model->update($branch, array('id' => $id)) )
		{	
			$this->db->commitTrans();
			//return status
			return new JStatus(true, 'branch details updated successfully') ;
		}
        $this->db->rollbackTrans();
		return new JStatus(false, 'Unable to update branch details') ;
	}
	/**
	 * Edit id
	 * 
	 * @param int $id
	 */
	function edit($id)
	{
		if( $this->input->post('btnSubmit') )
		{
			$jstat = $this->onEdit($id) ;

			if( $jstat->status )
			{
				ob_start() ;
				if( $this->getArg('contentAreaClicked') == 'idContentAreaSmall' )
				{
					$this->view($id) ;					
				}
				else
				{
					$this->page() ;
				}
				$jstat->data['idContentAreaBig'] = ob_get_clean() ;
			}
			$this->statusResponse($jstat) ;
			return false ;
		}

		$this->vars['mode'] = 'edit' ;
		$this->setArg('editId', $id) ;
		$this->vars['url'] = siteUrl('branch/edit' . '/' . $id ) ;
		$this->vars['result'] = $this->branch_model->getDetails($id) ;
		
		$this->vars['countries']=$this->getModel('country_model')->get();
		$this->vars['states']=$this->getModel('state_model')->getWhereBy( array('country_id' => $this->vars['result']['country_id']));
		$this->vars['cities']=$this->getModel('city_model')->getWhereBy( array('state_id' => $this->vars['result']['state_id']));

		
		$branchId = branchEmployee() ;
		$where = array() ;
		if( $branchId )
		{
			$where = array(
				'branch_id' => $branchId
			) ;
		}
		$this->vars['screens']=$this->getModel('screen_model')->getWhereBy( $where );
		
		$this->loadView('branch_add.php') ;
	}
	/**
	 * Display indivitual details
	 */
	function view($id)
	{
		$rec = $this->branch_model->getDetails($id) ;
		$this->loadView('branch_view.php', array('result' => $rec)) ;
	}
	/**
	 * Mark a record as deleted.
	 * 
	 * @param int $id id to remove
	 * @return bool
	 */
	function delete($id, $silent = false )
	{


		$sql = "SELECT count(*) as cnt FROM branch_group_entry WHERE branch_id='$id'" ;
		$hasLead = $this->db->scalarField($sql) ;
		if( $hasLead )
		{
			if( ! $silent )
			{
				$this->statusResponse( 'Fail', 'Unable to delete, Lead exists.', array('_id' => $id) ) ;
			}
			return false ;
		}


		$where = array(
			'id' => $id
		);

		$status = false ;

		if( $this->branch_model->delete($where) )
		{

			$status = true ;
		}
		if( ! $silent )
		{
			$this->statusResponse( (($status) ? 'OK' : 'Fail'), ( ($status) ? 'Branch deleted successfully' : 'Unable to delete branch group'), array('_id' => $id)) ;
		}
		return $status;
	}

	/**
	 * Mark a record as deleted.
	 * 
	 * @param int $id id to remove
	 * @return bool
	 */
	function group($branches, $silent = false )
	{
		$data = array(
			'name' => $this->fields['hidbulkprompt']
		) ;
		
		$this->db->beginTrans() ;
		$newId = 0 ;
		$this->loadModel('branch_group_entry_model') ;
		if( $this->loadModel('branch_group_model')->insert($data) )
		{
			$newId = $this->db->getLastInsertId() ;
			foreach( $branches as $one )
			{
				$dataone = array(
					'branch_group_id' => $newId,
					'branch_id' => $one,
				) ;
				$this->branch_group_entry_model->insert($dataone) ;
			}
			$status = true ;
			$this->db->commitTrans() ;
		}
		if( ! $silent )
		{
			$this->statusResponse( (($status) ? 'OK' : 'Fail'), ( ($status) ? 'Branch group created successfully' : 'Unable to create group'), array('_id' => $newId)) ;
		}
		return $status;
	}
	/**
	 * Buld action handler
	 * 
	 * @return boolean true on succes
	 */
	function bulkAction()
	{
		//Select action
		$action = @$this->fields['hidbulkaction'] ;

		//Validate bulk action
		if( ! $action )
		{
			$this->statusResponse('FAIL', 'Unknown action') ;
			return false ;
		}

		$branch = @$this->fields['cbList'] ;
		if( @count($branch) < 1 )
		{
			$this->statusResponse('FAIL', 'There are no branch') ;
			return false ;
		}

		//Do bulk action
		$msge = '' ;
		$stat = array('action-s' => 0, 'action-f' => 0) ;
		switch( $action )
		{
			case 'delete' :
				$msgs = '%s branch(s) deleted.' ;
				$msge = '%f branch(s) not deleted.' ;
				foreach( $branch as $v )
				{
					if( $this->delete($v, true) )
					{
						$stat['action-s'] ++ ;
					}
					else
					{
						$stat['action-f'] ++ ;
					}
				}
				break ;
			case 'group' :
				$msgs = 'Group created.' ;
				$msge = 'Group not created.' ;
				if( $this->group($branch, true) )
				{
					$stat['action-s'] = 1 ;
				}
				else
				{
					$stat['action-s'] = 0 ;
					$stat['action-f'] = 0 ;
				}
				break ;
		}

		//Format message
		$msg = '' ;
		$status = 'FAIL' ;
		if( $stat['action-s'] > 0 )
		{
			$msg = str_ireplace('%s', $stat['action-s'], $msgs) ;
			$status = 'OK' ;
		}
		if( $stat['action-f'] > 0 )
		{
			$msg = $msg . ' ' . str_ireplace('%f', $stat['action-f'], $msge) ;
			$status = 'FAIL' ;
		}

		$this->statusResponse($status, $msg) ;
	}
}
?>