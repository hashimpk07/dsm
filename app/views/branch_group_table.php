<form action="<?php echo siteUrl('branch_group/bulkaction');?>" method="post" id="frmList<?php echo get_class($this);?>" >
	...
	<div class="list-iconset">
		<?php if($this->accessAllowed('branch_group', 'delete') ) { ?>
		...
		<?php } ?>
		<?php if($this->accessAllowed('branch_group', 'group') ) { ?>
...
<table width="100%" style="float: left;" class="tab-grey">
	<tr>
		...
		<th class="m s" ><?php echo drawTh('Name', 'bg.name');?></th>
		...
		if( count($records) > 0 )
		{
			foreach ($records as $rec)
			{
				...
				<tr class="<?php echo $bg; ?>" id="idTableRow<?php echo $rec['id'];?>">
					<td><input name="cbList[]" value="<?php echo @$rec['id']; ?>" type="checkbox" class="listchk-group" /></td>
					<td class="v"><?php echo @$rec['qslno']; ?></td>
					<td class="m s v"><?php echo @$rec['name']; ?></td>
					<td class="m action-column">
	
						<?php if($this->accessAllowed('branch_group', 'edit') ) { ?>
						<span class="flaticon-edit" title="<?php echo l('Edit details');?>" onclick="actionEdit( '<?php echo siteUrl('branch_group/edit' . '/' . @$rec['id'] );?>', {}, 'idContentAreaDetect');" />
						<?php } ?>
						<?php if($this->accessAllowed('branch_group', 'delete') ) { ?>
						<span class="flaticon flaticon-trash" title="<?php echo l('Delete');?>" onclick="actionFlag( '<?php echo siteUrl('branch_group/delete' . '/' . @$rec['id'] );?>', {}, 'Are you sure you want to delete this group?', '', null, function(){htmlRefreshTable('idListArea<?php echo get_class($this);?>RefreshTable')}, true);" />
						<?php } ?>
					</td>
					<script type="text/javascript">
						jQuery('#idTableRow<?php echo $rec['id'];?> .v').on('click', function() { actionView( '<?php echo siteUrl('branch_group/view' . '/' . @$rec['id'] );?>', {}, 'idContentAreaDetect');} ) ;
					</script>
				</tr>
				<?php
			}
...
<script type="text/javascript">
	...
	bindCheckAll('.listchk-all', '.listchk-group:checkbox') ;
