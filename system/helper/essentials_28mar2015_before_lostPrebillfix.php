		...
		foreach( $recset as $rec )
		{
			...
			switch( $rec['af_unit'] )
			{
				...
				case 'D' :
					...
					break ;
				case 'M' :
					$vdBillAmount = _billAmountAcrossMonth($rec['af_last_dt'], $next_dt, $rec['af_unit_price'], $days_to_next) ;
					break ;
				case 'Y' :
...
function _billAmountAcrossMonth($from, $to, $unitPrice, $days_to_next )
{
	//if full month recived, then pass the unitPrice itself... {
	$diff_ts = strtotime($to) - strtotime($from) ;
	$diff = intval($diff_ts / 86400);
	echo "$diff == $days_to_next" ;
	if( $diff == $days_to_next )
	{
		return $unitPrice ; //WHAT IF BI MONTHLY BILLING ??? TODO 
	}
	//}
...
function postBill($dt = null)
{
	...
	$sql = "SELECT TIMESTAMPDIFF(HOUR, af_last_dt, af_end_dt) as process_flag, af_id, DATE_ADD(af_end_dt, INTERVAL 1 DAY) as af_end_dt_plus_one, ag_meeting,
					IF( DATE_ADD(af_last_dt, INTERVAL af_bill_cycle MONTH) > af_end_dt, true, false) as b_partial_m, 
					IF( DATE_ADD(af_last_dt, INTERVAL af_bill_cycle DAY) > af_end_dt, true, false) as b_partial_d, 
					TIMESTAMPDIFF( DAY, af_last_dt, $af_end_dt_plus_one) as days_to_end,
					TIMESTAMPDIFF( HOUR, af_last_dt, $af_end_dt_plus_one) as hours_to_end,
...
function invoke_process_outbpx($retry = false)
{
	if( $retry )
	{
		exec( 'wget -O /dev/null http://centre360.qudratom.com/admin.php/cron/process_outbox/1' ) ;
	}
	else
	{
		exec( 'wget -O /dev/null http://centre360.qudratom.com/admin.php/cron/process_outbox' ) ;
	}
}
?>